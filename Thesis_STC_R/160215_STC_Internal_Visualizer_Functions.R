...
STC_Internal_Visualization_Setup <-
  function(dataframe,STC_Title = "STC_Visualization",subtitle = NULL, add = FALSE, proj = "LL") {
    if (proj == "LL") {
      plot3d(
    ...
    if (proj == "UTM") {
      plot3d(
        0,0,0, xlim = range(dataframe$UTM.East),
        ylim = range(dataframe$UTM.North), zlim = range(dataframe$TimeDateNumeric),
        xlab = NULL,ylab = NULL,zlab = NULL,
        ticktype = "detailed", add = add
      )
      decorate3d(
        xlab = "Easting",ylab = "Northing",zlab = "Date",
        box = TRUE,axes = TRUE,main = STC_Title,sub = subtitle,top = TRUE,expand = 1.05
      )
    }
...
STC_Internal_Point_Line_Sphere_Visualizer <-
  function(dataframe,x,y,z, Type = "p",color = as.numeric(as.factor(dataframe$Identifier)), add = TRUE, size = 5, radius = 2, ...) {
    if (Type == "p" | Type == "l" | Type == "s" | Type == "sp") {
      if (Type == "p") {
        points3d(x, y, z, color = color,add = add, size = size, ...)
      }
      if (Type == "l") {
        lines3d(x, y, z, color = color,add = add, ...)
      }
      if (Type == "s") {
        spheres3d(x, y, z, color = color,add = add,radius = radius, ...)
      }
      if (Type == "sp") {
...
STC_Identify3d <- function(visualized_dataframe, proj = "LL") {
  if (proj == "LL") {
    xyzanimal.matrix <-
      matrix(
        c(
          visualized_dataframe$long,visualized_dataframe$lat,
          visualized_dataframe$TimeDateNumeric
        ),ncol = 3
      )
    identify3d(x = xyzanimal.matrix, labels = visualized_dataframe$Identifier,plot = TRUE)
  }
  if (proj == "UTM") {
    xyzanimal.matrix <-
      matrix(
        c(
          visualized_dataframe$UTM.East,visualized_dataframe$UTM.North,
          visualized_dataframe$TimeDateNumeric
        ),ncol = 3
      )
    identify3d(x = xyzanimal.matrix, labels = visualized_dataframe$Identifier,plot = TRUE)
